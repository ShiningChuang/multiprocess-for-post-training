#!/usr/bin/env python3
"""
V2i Interleaved Pipeline GRPO - 4-GPU, async generators + streaming learner.

Architecture (same physical layout as V2):
  cuda:0 = Learner (policy + ref + optimizer)
  cuda:1, cuda:2, cuda:3 = Generators (each with LoRA-equipped policy)

The async difference vs V2:
  * Generators free-run: generate → put → (non-blocking) check own weight_queue → repeat.
    They do NOT wait for the Learner to finish training.
  * Learner streams: pull one rollout from data_queue → train → repeat. Broadcasts
    weights every SYNC_EVERY_N_STEPS training steps by copying LoRA state_dict
    into each Generator's weight_queue. No NCCL barrier.

Each Generator runs exactly GEN_STEPS_PER_WORKER rollouts (10), then pushes a
None to the data_queue. Learner exits when it has seen NUM_GENERATORS Nones.

Comparison-critical new metrics (per Learner step):
  t_overlap  - time spent waiting at data_queue.get(). Should be ~0 in steady state.
  staleness  - learner_weight_version - rollout_weight_version. Higher = more off-policy.

All other hyperparams identical to V2 for direct comparison.
"""
...
LORA_ALPHA = 128
LOG_EVERY = 1
SYNC_EVERY_N_STEPS = 3   # match V2's broadcast cadence
SAVE_DIR = "/data/project/v2i_results"
...
def generator_worker(gen_id, device_idx, data_queue, weight_queue, all_data):
    """gen_id is 1..NUM_GENERATORS, device_idx is the cuda index."""
    try:
        ...
        # Stagger which prompts each Generator sees
        data_idx = (gen_id - 1) * BATCH_SIZE
        current_weight_version = 0

        for local_step in range(GEN_STEPS_PER_WORKER):
            # Non-blocking weight update — drain whatever is in the queue, apply the latest
            latest_state = None
            latest_version = None
            try:
                while True:
                    payload = weight_queue.get_nowait()
                    latest_state, latest_version = payload
            except Empty:
                pass
            if latest_state is not None:
                # Move to device and load
                gpu_state = {k: v.to(device) for k, v in latest_state.items()}
                model.load_state_dict(gpu_state, strict=False)
                current_weight_version = latest_version
                del gpu_state, latest_state
            ...
            print(f"[Generator-{gen_id}] step {local_step+1}/{GEN_STEPS_PER_WORKER}: "
                  f"gen={t_gen:.1f}s wver={current_weight_version}", flush=True)

        # Signal done
        data_queue.put(None)
        print(f"[Generator-{gen_id}] done, sent stop signal.", flush=True)
...
def learner_worker(data_queue, weight_queues):
    """weight_queues: list of mp.Queue, one per Generator."""
    try:
        device = "cuda:0"
        ...
        while True:
            ...
            if rollout is None:
                nones_received += 1
                print(f"[Learner] received stop signal {nones_received}/{NUM_GENERATORS}", flush=True)
                if nones_received >= NUM_GENERATORS:
                    break
                continue

            global_step += 1
            staleness = learner_weight_version - rollout["weight_version"]

            t_train_start = time.time()
            m = train_one_step(policy_model, ref_model, tokenizer, optimizer, rollout, device)
            t_train = time.time() - t_train_start

            # Sync weights every SYNC_EVERY_N_STEPS
            t_sync_start = time.time()
            if global_step % SYNC_EVERY_N_STEPS == 0:
                learner_weight_version += 1
                lora_state = {
                ...
                for q in weight_queues:
                    try:
                        q.put_nowait((lora_state, learner_weight_version))
                    except Exception:
                        # Queue full — Generator hasn't drained yet; ok, skip this fanout to that one
                        pass
            ...
            if global_step % LOG_EVERY == 0 or global_step == 1:
                print(f"[Learner] step {global_step:3d} (gen {rollout['gen_id']}) | "
                      ...
                      f"stale={staleness} (lver={learner_weight_version})",
                      flush=True)
...
def _sigterm_handler(signum, frame):
    print(f"[Main] received signal {signum}, terminating children...", flush=True)
    for p in _child_procs:
        try:
            ...
            pass
    # Don't sys.exit; let main run to its join cleanup
...
def main():
    ...
    print("=" * 60)
    print("V2i Interleaved Pipeline GRPO - 4-GPU")
    print("=" * 60)
    ...
    print(f"  Total steps: {NUM_TRAIN_STEPS} (each generator runs {GEN_STEPS_PER_WORKER})")
    ...
    manager = mp.Manager()
    data_queue = manager.Queue(maxsize=NUM_GENERATORS * 3)
    weight_queues = [manager.Queue(maxsize=2) for _ in range(NUM_GENERATORS)]

    # Launch Learner first
    learner_proc = mp.Process(target=learner_worker, args=(data_queue, weight_queues), name="Learner")
    learner_proc.start()
    ...
    for gid in range(1, NUM_GENERATORS + 1):
        device_idx = gid  # gen 1 → cuda:1, etc.
        p = mp.Process(target=generator_worker,
                       args=(gid, device_idx, data_queue, weight_queues[gid - 1], all_data),
    ...
    if os.path.exists(metrics_path):
        ...
        n = len(metrics)
        print("\n" + "=" * 60)
        print(f"Training Summary (V2i — {n} steps)")
        print("=" * 60)
        if n >= 10:
            early = metrics[:10]
            late = metrics[-10:]
        ...
        print(f"    t_sync:                 {avg('t_sync'):.3f}s ({avg('t_sync')/total*100:.0f}%)")
        ...
        print(f"  Generator wall (t_gen): {avg('t_gen'):.1f}s")
